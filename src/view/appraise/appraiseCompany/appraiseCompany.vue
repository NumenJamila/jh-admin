...
import { companyAppraiseList, companyAppraiseSave, companyAppraiseDelete } from "@/api/data";
...
export default {
            ...
            return h("div", [
              ...
              h(
                "Poptip",
                {
                  ...
                  on: {
                    "on-ok": () => {
                      this.CompanyAppraiseDelete(params.row.id);
                    }
                  }
                },
                [
  ...
  methods: {
    ...
    // 删除产业
    CompanyAppraiseDelete(id) {
      companyAppraiseDelete(id)
        .then(res => {
          this.loading = false;
          if (res.data.isSuccess) {
            this.$Message.info("已删除产业");
            this.CompanyAppraiseList();
          } else {
      ...
      this.$refs.formValidate.validate(valid => {
        if (valid) {
          if (this.formItem.domainNo == 0) {
            companyAppraiseSave(this.formItem)
              .then(res => {
                if (res.data.isSuccess) {
